#!/usr/bin/env python3
"""
Comprehensive Test for Refactored System

This script tests the entire refactored system including:
- Logging system
- Error handling
- Debug utilities
- Core modules
...
def test_cli_help():
    ...
    try:
        ...
        if result.returncode == 0:
            assert 'process' in result.stdout, "Should show process command"
            assert 'web' in result.stdout, "Should show web command"
            assert 'config' in result.stdout, "Should show config command"
        ...
        print(f"❌ CLI help tests failed: {e}")
        return False

def test_cli_process_help():
    """Test CLI process help command"""
    print("🧪 Testing CLI Process Help...")
    
    try:
        result = subprocess.run(
            [sys.executable, 'main.py', 'process', '--help'],
            capture_output=True,
            text=True,
            timeout=10
        )
        ...
        if result.returncode == 0:
            assert '--input' in result.stdout, "Should show input option"
            assert '--output' in result.stdout, "Should show output option"
            assert '--clip-modes' in result.stdout, "Should show clip-modes option"
            print("✅ CLI process help tests passed!")
            return True
        else:
            print(f"❌ CLI process help failed with exit code: {result.returncode}")
            return False
            
    except Exception as e:
        print(f"❌ CLI process help tests failed: {e}")
        return False
